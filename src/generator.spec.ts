...
describe('PrismaClassGenerator#resolvePrettierOptions — 사용자 prettier 설정 감지', () => {
	// README에 문서화된 동작: "생성된 클래스는 (있다면) 사용자의 prettier 설정 파일을
	// 사용해 포맷된다." output 디렉토리부터 상위로 올라가며 .prettierrc 등을 찾는다.
	// prettier 3부터 resolveConfig가 async 전용이라 이 감지도 async로 수행한다.
	it('output 디렉토리 상위에 있는 .prettierrc를 찾아서 적용한다', async () => {
		...
		await generator.resolvePrettierOptions()
	...
	it('output 디렉토리가 아직 생성되지 않았어도 상위의 prettier 설정을 찾는다', async () => {
		const projectDir = fs.mkdtempSync(
			path.join(os.tmpdir(), 'pcg-prettier-test-'),
		)
		fs.writeFileSync(
	...
	// null을 반환해야 한다.
	it('prettier 설정이 어디에도 없어도 크래시 없이 null을 반환한다', async () => {
		const projectDir = fs.mkdtempSync(
			path.join(os.tmpdir(), 'pcg-prettier-test-'),
		)
		...
		expect(generator.prettierOptions).toBeNull()
